...
trDs = tfDataProcessing.getTiffTrainDataSet(cytoImagePath,trIdents,trLabels) \
    .map(tfDataProcessing.loadTiffImage, num_parallel_calls=tf.data.experimental.AUTOTUNE) \
    .filter(tfDataProcessing.isValidPack) \
...
valDs = tfDataProcessing.getTiffTrainDataSet(cytoImagePath,vaIdents,vaLabels) \
    .map(tfDataProcessing.loadTiffImage, num_parallel_calls=tf.data.experimental.AUTOTUNE) \
    .filter(tfDataProcessing.isValidPack) \
    .map(downscale, num_parallel_calls=tf.data.experimental.AUTOTUNE) \
    .map(coerceSeqSize, num_parallel_calls=tf.data.experimental.AUTOTUNE) \
